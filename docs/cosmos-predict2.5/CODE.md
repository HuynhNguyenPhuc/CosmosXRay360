...
**Main Inference Package:** `predict2_5/`  
**Foundation Submodule Backbone:** `cosmos-predict2.5/` (`cosmos_predict2/`)  
**Model Wrappers:** `predict2_5/wrapper.py` (`CosmosPredict25Wrapper`) & `baselines/models/cosmos.py`  
**Evaluation Harnesses:** `predict2_5/evaluate.py` (`CosmosEvaluator`) & `baselines/evaluate.py`  
...
### A. Core Package (`predict2_5/`)

```
predict2_5/
├── constants.py       # Core constants: NUM_FRAMES=93, VOL_SIZE=256, UUIDs, prompts, dims
├── datamodule.py      # PreRendered360Dataset & PreRenderedDataModule (93-view PNG loader)
├── inferencer.py     # Inferencer pipeline (DiT init, UniPC sampling, Frame Replacement)
├── wrapper.py        # CosmosPredict25Wrapper conforming to unified NVS baseline API
├── evaluate.py       # Standalone evaluation harness (PSNR, SSIM, LPIPS, Latency)
├── module.py         # CosmosXRay360 PyTorch Lightning Module (Rectified Flow, EMA, CFG)
...
└── imaginaire/
    ...
    └── lazy_config.py            # LazyDict, LazyCall hydra utilities
```
...
where $\gamma$ is solved from cubic polynomial $s^2 (12 + 16\gamma + 7\gamma^2 + \gamma^3) = (1 + \gamma)^3$ for rate $s = 0.10$.
...
This table verifies `predict2_5/module.py` against NVIDIA's official post-training configuration (`model_2B_reason_1p1_rectified_flow.py` & `Video2WorldModelRectifiedFlowConfig`):

| Parameter | Official NVIDIA Post-Training Config | `predict2_5/module.py` Implementation | Verified? |
|---|---|---|:---:|
| **DiT Backbone Class** | `MinimalV1LVGDiT` | `MinimalV1LVGDiT` (`_create_dit`) | ✅ |
| **Model Size Channels** | `model_channels=2048`, `num_blocks=28`, `num_heads=16` | `2B`: `{2048, 28, 16}` (`_get_model_config`) | ✅ |
| **3D RoPE Extrapolation** | `rope_h=3.0`, `rope_w=3.0`, `rope_t=1.0` | `rope_h=3.0`, `rope_w=3.0`, `rope_t=1.0` | ✅ |
| **Cross-Attention Dim** | `crossattn_proj_in_channels=100352` | `CROSSATTN_PROJ_IN_CHANNELS = 100352` | ✅ |
| **Cross-Attention Proj** | `use_crossattn_projection=True`, `channels=1024` | `use_crossattn_projection=True`, `channels=1024` | ✅ |
| **RF Shift Factor** | `shift = 5` | `rf_shift = 5.0` | ✅ |
| **Time Sampling** | `train_time_distribution = "logitnormal"` | `"logitnormal"` | ✅ |
| **Time Weighting** | `"uniform"` | `"uniform"` | ✅ |
| **Conditioning Strategy** | `FRAME_REPLACE` | Latent & velocity splicing in `denoise()` | ✅ |
| **Conditioning Frames** | `min=1`, `max=2` (Latent frame count) | `min_num_conditional_frames=1`, `max=2` | ✅ |
| **CFG Dropout Rate** | `text.dropout_rate = 0.2` | `cfg_dropout_rate = 0.2` (per-sample Bernoulli) | ✅ |
| **Optimizer** | `fusedadamw` / `fusedadam` | `get_base_optimizer(optim_type="fusedadam")` | ✅ |
| **Learning Rate** | `lr = 2**(-14.5)` ($\approx 4.3158 \times 10^{-5}$) | `learning_rate = 2**(-14.5)` | ✅ |
| **Weight Decay** | `weight_decay = 0.001` | `weight_decay = 0.001` | ✅ |
| **LR Warmup Steps** | `warm_up_steps = 2000` | `warmup_steps = 2000` (`LinearLR` + `Cosine`) | ✅ |
| **Power EMA Rate** | `ema_rate = 0.10` | `ema_rate = 0.10` (`FastEmaModelUpdater`) | ✅ |
| **Precision** | `bfloat16` (`bf16-mixed`) | `bf16-mixed` / `torch.autocast(dtype=bfloat16)` | ✅ |
...
4. **Multi-View Integration Wrapper (`CosmosPredict25Wrapper`):**
   - Exposes standardized `infer_multi_views(input_xray, azimuths=(0, 360, 93))` method.
   - Converts input tensors, NumPy arrays, or PIL images into normalized $[0, 1]$ 2D PyTorch tensors `[93, 1, 256, 256]`, seamlessly fitting into `baselines/evaluate.py`.

---

## 7. Entrypoints, Testing & Verification Commands

### A. Test DataModule Loading
```bash
PYTHONPATH=. uv run python predict2_5/datamodule.py --data-dir datasets/pre_rendered
```

### B. Run Main Method Evaluation (NSCLC OOD Test Split)
```bash
python predict2_5/evaluate.py --checkpoint-path "hf://phuchuynh0904/CosmosXRay360/net_ema.pth" --max-patients 10
```

### C. Run Unified 7-Model Evaluation Harness
```bash
python baselines/evaluate.py
```

### D. Run Wrapper Unit Tests
```bash
python -m pytest predict2_5/tests/test_cosmos_wrapper.py -v
```
