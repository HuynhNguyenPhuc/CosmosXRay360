...
try:
    ...
    logging.getLogger(__name__).warning(
        "drr_projector_function CUDA extension is not importable for this Python "
        "interpreter (rebuild it with `python setup.py install` from "
        "xraysyn/networks/drr_projector/ under the active venv). Falling back to a "
        "stub that returns all-zero volumes/projections -- XRaySyn forward/back "
        "projection is a no-op until the extension is rebuilt."
    )

    class DummyDRRFunction:
        def __getattr__(self, name):
            def dummy_fn(*args, **kwargs):
                if "backward" in name or "backproj" in name:
                    # Return 3D volume tensor fallback
                    if len(args) > 1 and isinstance(args[1], (list, tuple, torch.Tensor)):
                        v_shape = tuple(args[1].tolist() if isinstance(args[1], torch.Tensor) else args[1])
                        return torch.zeros((args[0].shape[0], args[0].shape[1], *v_shape), device=args[0].device)
                    return torch.zeros((args[0].shape[0], args[0].shape[1], 128, 128, 128), device=args[0].device)
                else:
                    # Return 2D projection tensor fallback
                    det_shape = args[1] if len(args) > 1 else (128, 128)
                    if isinstance(det_shape, torch.Tensor):
                        det_shape = tuple(det_shape.tolist())
                    return torch.zeros((args[0].shape[0], args[0].shape[1], *det_shape), device=args[0].device)
            return dummy_fn
    drr_projector_function = DummyDRRFunction()
